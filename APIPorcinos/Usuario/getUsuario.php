<?php 
    ...
    if (!empty($_POST['correo_usuario']) && !empty($_POST['password_usuario'])) {
	    $consultaAdmin = $base_de_datos->query("SELECT * FROM administrador WHERE correo = '".$_POST['correo_usuario']."' AND passw = '".$_POST['password_usuario']."'");
	    $datosAdmin = $consultaAdmin->fetchAll();

        $consultaPorci = $base_de_datos->query("SELECT * FROM porcicultor WHERE correo = '".$_POST['correo_usuario']."' AND passw = '".$_POST['password_usuario']."'");
